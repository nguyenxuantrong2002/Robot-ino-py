...
class send_command:
    ...
    def send_data(self, th2, th3, PulseX, PulseY):
        try:
            msg = "G" +  str(th2) + "B" + str(th3) + "C" +  str(PulseX)+"X"+str(PulseY)+"YZ"
    ...
    def send_manual(self, th1, th2, th3, th4, th5):
        try:
            msg = "F" + str(th1) + "A" + str(th2) + "B" + str(th3) + "C" + str(th4) + "D" + str(th5) + "E"
            self.ser.write(msg.encode())
            self.ser.flushOutput()
            print("Da truyen xong du lieu")
        except:
...
class MainWindow:
    ...
    def __init__(self):
        ...
        """****"""
        self.type1_pulseA, self.type1_pulseB, self.type1_pulseC = self.robot_init.InvKinematics(10,-24,10)
        self.type1_th1, self.type1_th2, self.type1_th3 = self.robot_init.CurrentPos_deg()
        """****"""
        self.type2_pulseA, self.type2_pulseB, self.type2_pulseC = self.robot_init.InvKinematics(-1, -24, 10)
        self.type2_th1, self.type2_th2, self.type2_th3 = self.robot_init.CurrentPos_deg()
        """****"""
        self.type3_pulseA, self.type3_pulseB, self.type3_pulseC = self.robot_init.InvKinematics(-12,-20,10)
        self.type3_th1, self.type3_th2, self.type3_th3 = self.robot_init.CurrentPos_deg()
    ...
    def control_manual(self,x):
        if self.flag_manual == True:
            ...
            if x == 7:
                self.serial.send_manual(0, 0, 0, -20, 0)
            if x == 8:
                self.serial.send_manual(0,0,0,0,20)
            if x == 9:
                self.serial.send_manual(0,0,0,0,-20)
    ...
    def CalibPos(self):
        try:
            x = float(self.uic.x_calib.text())
            y = float(self.uic.y_calib.text())
            z = float(self.uic.z_calib.text())
            #print(x,y,z)
            current_pulseA, current_pulseB, current_pulseC = self.robot_init.InvKinematics(x,y,z)
            self.serial.send_manual((current_pulseA -self.home_pulseA), (current_pulseB-self.home_pulseB) , -(current_pulseC-self.home_pulseC),0,0)
    ...
    def PickObject(self,x,y,z,type):
        ...
        self.serial.send_manual(self.home_pulseA - pick_pulseA, 0,0, int(round(th1-90,0)),0)
        self.serial.send_manual(0,-(self.home_pulseB - pick_pulseB), 0,0,90)
        self.setvalue(x,y,z,th1,th2,th3)

        self.serial.send_manual(0, -(pick_pulseB - self.home_pulseB), 0, int(round(90-th1,0)), 0)
        pick_pulseB = self.home_pulseB
        pick_pulseC = self.home_pulseC
        if type ==1:
            self.serial.send_manual(pick_pulseA - self.type1_pulseA, -(pick_pulseB - self.type1_pulseB),
                                    (pick_pulseC - self.type1_pulseC), 0,-90)
            self.setvalue(10,-24,10,self.type1_th1,self.type1_th2,self.type1_th3)
            self.serial.send_manual(0, -(self.type1_pulseB - self.home_pulseB),
                                    self.type1_pulseC - self.home_pulseC, 0, 0)
            self.serial.send_manual(self.type1_pulseA - self.home_pulseA, 0, 0, 0, 0)
            self.setvalue(self.home_x,self.home_y,self.home_z,self.home_th1,self.home_th2,self.home_th3)
        if type == 2:
            self.serial.send_manual(pick_pulseA - self.type2_pulseA, -(pick_pulseB - self.type2_pulseB),
                                      0, 0, -90)
            self.setvalue(-1, -24, 10, self.type2_th1, self.type2_th2, self.type2_th3)
        ...
        if type == 3:
            self.serial.send_manual(pick_pulseA - self.type3_pulseA, -(pick_pulseB - self.type3_pulseB),
                                    (pick_pulseC - self.type3_pulseC), 0,-90)
            self.setvalue(-1, -24, 10, self.type3_th1, self.type3_th2, self.type3_th3)
            ...
            self.setvalue(self.home_x, self.home_y, self.home_z, self.home_th1, self.home_th2, self.home_th3)
    def setvalue(self,x,y,z,th1,th2,th3):
        self.uic.x_current.setText(str(x))
        self.uic.y_current.setText(str(y))
        self.uic.z_current.setText(str(z))
        self.uic.th1_current.setText(str(th1))
        self.uic.th2_current.setText(str(th2))
        self.uic.th3_current.setText(str(th3))
    ...
    def main_fcn(self):
        ...
        for cnt in contours:
            ...
            if area > 10000:
                ...
                T = self.robot_init.TransferCoor(18.3,3.4,0,0,0,0)
                Coor = T@Coor
                self.y_cm = Coor[1,0]
                self.x_cm = Coor[0,0]
        ...
        for barcode in decode(belt):
                ...
                if self.QRcode_data != self.pre_QRcode_data:
                    ...
                    self.flag_data = False
        scale_percent = 86  # percent of original size
        ...
        if self.flag_data == True:
                ...
                self.PickObject(self.x_cm,self.y_cm-8,12.5,self.command)
